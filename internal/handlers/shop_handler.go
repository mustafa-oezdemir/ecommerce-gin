...
type ShopHandler struct { cartService *services.CartService; orderService *services.OrderService }

func NewShopHandler() *ShopHandler { return &ShopHandler{cartService: services.NewCartService(), orderService: services.NewOrderService()} }
...
func (h *ShopHandler) renderProducts(c *gin.Context) {
	...
	if categoryID, err := strconv.ParseUint(c.Query("category"), 10, 64); err == nil && categoryID > 0 { query = query.Where("category_id = ?", uint(categoryID)) }
	if err := query.Find(&products).Error; err != nil { c.String(http.StatusInternalServerError, "Could not load products"); return }
...
func (h *ShopHandler) ProductDetail(c *gin.Context) {
	...
	if err != nil || id == 0 { c.AbortWithStatus(http.StatusNotFound); return }
	var product models.Product
	if err := db.DB.Preload("Category").Where("id = ? AND active = ?", uint(id), true).First(&product).Error; err != nil { c.AbortWithStatus(http.StatusNotFound); return }
	c.HTML(http.StatusOK, "product_detail.tmpl", viewData(c, gin.H{"Product": product}))
}

func (h *ShopHandler) AddToCart(c *gin.Context) {
	user, ok := middleware.CurrentUser(c); if !ok { c.AbortWithStatus(http.StatusUnauthorized); return }
	...
	if err := c.ShouldBind(&req); err != nil { c.String(http.StatusBadRequest, "Invalid quantity"); return }
	if err := h.cartService.AddToCart(*user, uri.ID, req.Quantity); err != nil { c.String(http.StatusBadRequest, "Could not add product to cart"); return }
...
func (h *ShopHandler) ViewCart(c *gin.Context) {
	...
	if err != nil { c.String(http.StatusInternalServerError, "Could not load cart"); return }
	var totalCents int64
	for _, item := range cart.Items { totalCents += item.Product.PriceCents * int64(item.Quantity) }
...
func (h *ShopHandler) UpdateCartItem(c *gin.Context) {
	...
	if err := h.cartService.UpdateQuantity(*user, uri.ID, req.Quantity); err != nil {
		if errors.Is(err, services.ErrCartItemNotFound) { c.AbortWithStatus(http.StatusNotFound); return }
...
func (h *ShopHandler) Checkout(c *gin.Context) {
	...
	if err != nil {
		switch {
		case errors.Is(err, services.ErrCartNotFound), errors.Is(err, services.ErrCartEmpty), errors.Is(err, services.ErrInvalidQuantity): c.String(http.StatusBadRequest, "Cart cannot be checked out")
		case errors.Is(err, services.ErrProductUnavailable), errors.Is(err, services.ErrInsufficientStock): c.String(http.StatusConflict, "A product is unavailable")
		default: c.String(http.StatusInternalServerError, "Checkout failed")
		}
		return
	}
...
func (h *ShopHandler) ListOrders(c *gin.Context) {
	...
	if err != nil { c.String(http.StatusInternalServerError, "Could not load orders"); return }
