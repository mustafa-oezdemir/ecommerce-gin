...
func NewOrderService() *OrderService { return &OrderService{orderRepo: repositories.NewOrderRepository()} }

func (s *OrderService) CreateOrder(user models.User) (*models.Order, error) {
	if user.ID == 0 { return nil, ErrInvalidUser }
	...
	err := db.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).Where("user_id = ?", user.ID).First(&cart).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) { return ErrCartNotFound }
			return fmt.Errorf("load cart: %w", err)
		}
		var cartItems []models.CartItem
		if err := tx.Where("cart_id = ?", cart.ID).Find(&cartItems).Error; err != nil { return fmt.Errorf("load cart items: %w", err) }
		...
		for _, cartItem := range cartItems {
			if cartItem.Quantity < 1 || cartItem.Quantity > 100 { return ErrInvalidQuantity }
			var product models.Product
			if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&product, cartItem.ProductID).Error; err != nil {
				if errors.Is(err, gorm.ErrRecordNotFound) { return ErrProductUnavailable }
				return fmt.Errorf("load product: %w", err)
			}
			if !product.Active { return ErrProductUnavailable }
			if product.Stock < cartItem.Quantity { return ErrInsufficientStock }
			subtotal := product.PriceCents * int64(cartItem.Quantity)
			if subtotal <= 0 || totalCents > (1<<63-1)-subtotal { return fmt.Errorf("invalid order total") }
			totalCents += subtotal
		...
		if err := tx.Create(&order).Error; err != nil { return fmt.Errorf("create order: %w", err) }
		for i := range items {
			items[i].OrderID = order.ID
			if err := tx.Create(&items[i]).Error; err != nil { return fmt.Errorf("create order item: %w", err) }
			...
			if result.Error != nil { return fmt.Errorf("decrement stock: %w", result.Error) }
			if result.RowsAffected != 1 { return ErrInsufficientStock }
		}
		if err := tx.Where("cart_id = ?", cart.ID).Delete(&models.CartItem{}).Error; err != nil { return fmt.Errorf("clear cart: %w", err) }
		order.Items = items
		created = order
		return nil
	})
...
func (s *OrderService) UpdateStatus(orderID uint, status models.OrderStatus) error {
	if orderID == 0 { return ErrInvalidTransition }
	return db.DB.Transaction(func(tx *gorm.DB) error {
		var order models.Order
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&order, orderID).Error; err != nil { return err }
		if !CanTransitionOrderStatus(order.Status, status) { return ErrInvalidTransition }
