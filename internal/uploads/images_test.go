...
func TestImageStoreAcceptsJPEGPNGAndWEBPAndNormalizesContent(t *testing.T) {
	store, err := NewImageStore(ImageConfig{Directory: t.TempDir(), MaxBytes: 1024 * 1024, MaxWidth: 100, MaxHeight: 100, MaxPixels: 10_000, Scanner: scannerFunc(func(context.Context, []byte) error { return nil })})
	if err != nil {
		t.Fatalf("create image store: %v", err)
	}
	...
	if err := jpeg.Encode(&jpegOutput, picture, nil); err != nil {
	...
	for _, test := range []struct {
		...
		{name: "avatar.jpg", data: jpegOutput.Bytes()},
		{name: "avatar.png", data: testPNG(t, 2, 2)},
		{name: "avatar.webp", data: webpData},
	} {
		t.Run(test.name, func(t *testing.T) {
			filename, err := store.Save(t.Context(), test.name, bytes.NewReader(test.data), int64(len(test.data)))
			if err != nil {
