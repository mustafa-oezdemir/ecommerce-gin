...
// Run composes and supervises the application and private metrics servers.
func Run() (runErr error) {
	cfg := config.Load()
	logRuntime, err := logging.New(logging.Config{
		Environment: cfg.AppEnv, Level: cfg.LogLevel, ConsoleFormat: cfg.LogConsoleFormat,
		FilePath: cfg.LogFile, MaxSizeMB: cfg.LogMaxSizeMB, MaxBackups: cfg.LogMaxBackups,
		MaxAgeDays: cfg.LogMaxAgeDays, Compress: cfg.LogCompress, AddSource: cfg.LogAddSource,
	})
	if err != nil {
	...
	imageStore, err := uploads.NewImageStore(uploads.ImageConfig{
		Directory: cfg.ProductImageDirectory, MaxBytes: cfg.ProductImageMaxBytes, MaxWidth: cfg.ProductImageMaxWidth,
		MaxHeight: cfg.ProductImageMaxHeight, MaxPixels: cfg.ProductImageMaxPixels, Scanner: malwareScanner,
	})
	if err != nil {
		return fmt.Errorf("configure product image storage: %w", err)
	}
	profileImageStore, err := uploads.NewImageStore(uploads.ImageConfig{
		Directory: cfg.ProfileImageDirectory, MaxBytes: cfg.ProfileImageMaxBytes, MaxWidth: cfg.ProfileImageMaxWidth,
		MaxHeight: cfg.ProfileImageMaxHeight, MaxPixels: cfg.ProfileImageMaxPixels, Scanner: malwareScanner,
	})
	if err != nil {
	...
	applicationHandler, err := appserver.NewRouter(appserver.RouterConfig{
		Environment: cfg.AppEnv, PublicURLHost: cfg.AppURLHost, TrustedProxies: cfg.TrustedProxies,
		SessionSecret: cfg.SessionSecret, SessionSecure: cfg.SessionSecure, CSRFKey: cfg.CSRFKey,
		SecurityKey: cfg.SecurityEncryptionKey, WebhookSecret: cfg.PaymentWebhookSecret, Database: database,
		Metrics: appMetrics, Logger: slog.Default(), ImageStore: imageStore, ProfileImageStore: profileImageStore,
		LogReader: logReader, ShippingClient: shippingClient, ShippingCallbackToken: cfg.ShippingCallbackToken,
	...
	serverGroup, err := appserver.NewGroup(
		":"+cfg.AppPort, applicationHandler, ":"+cfg.MetricsPort, promhttp.Handler(),
		appserver.HTTPConfig{ReadHeaderTimeout: cfg.HTTPReadHeaderTimeout, ReadTimeout: cfg.HTTPReadTimeout,
			WriteTimeout: cfg.HTTPWriteTimeout, IdleTimeout: cfg.HTTPIdleTimeout, ShutdownTimeout: cfg.HTTPShutdownTimeout,
			MaxHeaderBytes: cfg.HTTPMaxHeaderBytes}, slog.Default(),
	)
	if err != nil {
