...
func registerRoutes(router *gin.Engine, database *gorm.DB, appMetrics *metrics.Metrics, imageStore, profileImageStore *uploads.ImageStore, logReader *logging.Reader, securityKey []byte, environment, webhookSecret string, shippingClient shippingapi.Client, shippingCallbackTokens ...string) {
	...
	customer.GET("/account/addresses/new", checkout.NewAddress)
	customer.GET("/account/addresses/:id/edit", checkout.EditAddress)
	...
	adminGroup.GET("/users/new", admin.NewUser)
	adminGroup.GET("/users/:id", admin.ViewUser)
	adminGroup.GET("/users/:id/edit", admin.EditUser)
	...
	adminGroup.GET("/orders/:id", admin.ViewOrder)
	adminGroup.GET("/categories", admin.ListCategories)
	adminGroup.GET("/categories/new", admin.NewCategory)
	adminGroup.GET("/categories/:id", admin.ViewCategory)
	adminGroup.GET("/categories/:id/edit", admin.EditCategory)
	...
	employeeGroup.GET("/products/new", employee.NewProduct)
	employeeGroup.GET("/products/:id", employee.ViewProduct)
	employeeGroup.GET("/products/:id/edit", employee.EditProduct)
