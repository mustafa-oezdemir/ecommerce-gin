...
func TestSharedShellRendersBrandAndRoleNavigation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "customer", data: map[string]any{"CurrentUser": &models.User{Name: "Ada Lovelace", Email: "ada@example.com", Role: models.RoleCustomer}, "CSRFField": template.HTML("csrf"), "ActiveNav": "orders"}, want: []string{`href="/cart"`, `class="account-dropdown"`, `class="account-avatar"`, `>AL</span>`, "Ada Lovelace", "ada@example.com", `href="/account"`, `href="/account/orders" aria-current="page"`, `href="/account/lists"`, `action="/logout"`, `class="site-nav-menu"`, `class="site-menu-toggle"`}, doNotWant: []string{`href="/login"`, `aria-hidden="true">⌄`}},
